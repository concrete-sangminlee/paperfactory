...
import re
from utils.quality_checker import check_paper, _count_body_words
...
def review_paper(paper_content: dict, journal_key: str, figures: list = None) -> dict:
    ...
    for exp in expected:
        if not any(exp in sn for sn in section_names):
            comments.append(_comment("major", "Structure",
    ...
    if intro:
        ...
        if intro_words < 500:
            comments.append(_comment("major", "Introduction",
        ...
        if not any(phrase in intro_text.lower() for phrase in ["this study", "this paper", "this work", "the present"]):
            comments.append(_comment("minor", "Introduction",
                "Introduction should explicitly state the objectives/contributions of this study."))
        if not any(phrase in intro_text.lower() for phrase in ["gap", "limitation", "however", "despite", "lacking"]):
            comments.append(_comment("major", "Introduction",
    ...
    if method:
        ...
        if "random" in method_text.lower() and "seed" not in method_text.lower():
            comments.append(_comment("minor", "Methodology",
                "If random processes are used, specify the random seed for reproducibility."))
        if not any(w in method_text.lower() for w in ["cross-validation", "validation", "test set", "holdout"]):
            comments.append(_comment("major", "Methodology",
    ...
    if results:
        ...
        if not any(w in results_text.lower() for w in ["table", "fig.", "figure"]):
            comments.append(_comment("major", "Results",
                "Results should reference specific tables and figures to support claims."))
        if not any(w in results_text.lower() for w in ["compar", "outperform", "superior", "baseline", "prior"]):
            comments.append(_comment("major", "Results",
                "Results should compare with existing methods or baselines."))
        if not any(w in results_text.lower() for w in ["r2", "r-squared", "rmse", "accuracy", "f1", "mae", "mse"]):
            comments.append(_comment("minor", "Results",
    ...
    if conclusion:
        ...
        if not any(w in conc_text.lower() for w in ["limitation", "future", "further"]):
            comments.append(_comment("minor", "Conclusions",
    ...
    # 6. References
    if len(refs) < 15:
        comments.append(_comment("major", "References",
    ...
    if refs and recent_count / len(refs) < 0.3:
        comments.append(_comment("minor", "References",
    ...
    if n_figs < 4:
        comments.append(_comment("minor", "Figures",
            f"Only {n_figs} figures. Consider adding more visualizations to support your analysis."))
    if n_tables < 2:
        comments.append(_comment("minor", "Tables",
            f"Only {n_tables} tables. Include comparison tables for model performance."))

    # 8. Abstract
    abstract_words = len(abstract.split())
    if abstract_words < 100:
        comments.append(_comment("minor", "Abstract",
            f"Abstract is very brief ({abstract_words} words). Include purpose, methods, key results, "
            "and conclusions."))
    if not any(w in abstract.lower() for w in ["result", "found", "achieved", "show", "demonstrate"]):
        comments.append(_comment("minor", "Abstract",
            "Abstract should include specific quantitative results."))

    # 9. Novelty
    all_text = " ".join(_get_full_text(s) for s in sections)
    if not any(phrase in all_text.lower() for phrase in ["novel", "first time", "for the first", "contribution", "unique", "new approach", "proposed"]):
        comments.append(_comment("major", "Novelty",
    ...
    return {
        ...
        "summary": (
            ...
            + ("\n".join(f"  [{c['severity'].upper()}] ({c['section']}) {c['comment']}" for c in comments))
        ),
    }
